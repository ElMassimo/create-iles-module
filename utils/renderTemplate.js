 ...
 */
function renderTemplate(src, dest, pkgName) {
  ...
  if (stats.isDirectory()) {
    ...
    for (const file of fs.readdirSync(src)) {
      renderTemplate(path.resolve(src, file), path.resolve(dest, file), pkgName)
    }
    return
  }
  ...
  if (content.includes('_PACKAGE_NAME_'))
    fs.writeFileSync(dest, content.replace(/_PACKAGE_NAME_/g, pkgName))
  else
